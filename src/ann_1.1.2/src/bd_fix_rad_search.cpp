...
void ANNbd_shrink::ann_FR_search(ANNdist box_dist, int core)
{
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_FR_search(inner_dist, core);// search inner child first
		child[ANN_OUT]->ann_FR_search(box_dist, core);// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_FR_search(box_dist, core);// search outer child first
		child[ANN_IN]->ann_FR_search(inner_dist, core);// ...then outer child
	}
