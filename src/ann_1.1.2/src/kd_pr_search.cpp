...
void ANNkd_tree::annkPriSearch(
	...
	int					core)			// 2021-12-08: added for pthread
{
...
std::cout << "annkPriSearch involved !!!\n";
	...
	while (ANNprBoxPQ->non_empty() &&
		...
		np->ann_pri_search(box_dist, core);	// search this subtree.
	}
...
void ANNkd_split::ann_pri_search(ANNdist box_dist, int core)
{
	...
	if (cut_diff < 0) {					// left of cutting plane
		...
		child[ANN_LO]->ann_pri_search(box_dist, core);
	}
		...
		if (child[ANN_LO] != KD_TRIVIAL)// enqueue if not trivial
			ANNprBoxPQ->insert(new_dist, child[ANN_LO]);
										// continue with closer child
		child[ANN_HI]->ann_pri_search(box_dist, core);
	}
...
void ANNkd_leaf::ann_pri_search(ANNdist box_dist, int core)
{
